...
def test_add_photo_of_pet(pet_photo="images\cat1.jpg")
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
    dir_pet_photo = os.path.join(os.path.dirname(__file__), pet_)photo
    pet_id = all_pets['pets'][0]['id']
    status, result = pf.add_photo_of_pet(auth_key, pet_id, dir_pet_photo)
    assert status == 200
    assert result['pet_photo'] == dir_pet_photo